/**************************************************************
 Cinema Management program in C by Nikolaos Perris (#36261)
    ...
    for Introduction to Algorithms and Programming
        ...
        36261@edu.ufp.pt / nperris@gmail.com
**************************************************************/
...
void menu_title(int w) // The top part of the menus with name/version
{
    ...
    printf("%c",187);
    printf("\n\t\t%c",186);
    for (int a=0;a<w;a++) printf(" ");
    printf("%c",186);
    gotoxy(w/2+6,2);printf("Cinema Management v0.99c");
    printf("\n\t\t%c",204);
    for (int a=0;a<w;a++) printf("%c",205);
    printf("%c",185);
    printf("\n\t\t%c",186);
    for (int a=0;a<w;a++) printf(" ");
    printf("%c",186);
    gotoxy(w/2+6,4);printf("(c) 2018 Nikolaos Perris");
    printf("\n\t\t%c",200);
    for (int a=0;a<w;a++) printf("%c",205);
    printf("%c\n\n",188);
}
...
void edit_cinema(struct cinema * m)  // Edit an existing cinema that the user selects
{
    ...
    if (i>0 && i<=num_cinemas) {                       // Checks if the option is valid
        ...
        if (ac=='Y' || ac=='y') {
            ...
            tg[strcspn(tg, "\n")] = 0;
            if (tg[0]==0) {                                     // checks if user typed anything at all
                printf("\n\t\tYou typed nothing!");
                no_changes();
                empt++;
            }
            for (j=0;j<num_cinemas;j++) {
                if ((strcmp(tg,vec_cinemas[j].c_name) == 0) && (strcmp(tg,ct) != 0)) exst++; // if new name is found in other entries and is different than the current name, then increase the counter
            }
            if (exst>0) {                    // if the counter is bigger than zero, then a duplicate entry was found, a message is displayed and nothing is changed
                printf("\n\t\tCinema '%s' already exists!",tg);
                no_changes();
            }
            else if (exst==0 && empt==0) {              // if the counter is zero, the temporary cinema name is copied on the place of the cinema name we selected
...
void create_movie(struct movie * m)  // Create a new movie entry
{
    ...
    if (chkg==0 && m->genre[0]!=0) {                      // If it doesn't exist, it gets added
...
void edit_movie(struct movie * m)  // Edit an existing movie that the user selects
{
    int i,j,ac,exst=0,empt=0,chkg=0;
    ...
    if (i>0 && i<=num_movies) {                                // Checks if the option is valid
        ...
        if (ac=='Y' || ac=='y') {
            ...
            for (j=0;j<num_movies;j++) {
                if ((strcmp(tg,vec_movies[j].m_title) == 0) && (strcmp(tg,ct) != 0)) exst++; // if new name is found in other entries and is different than the current name, then increase the counter
            }
            if (exst>0) {                    // if the counter is bigger than zero, then a duplicate entry was found, a message is displayed and nothing is changed
                printf("\n\t\tMovie '%s' already exists!",tg);
                no_changes();
            }
            else if (exst==0 && empt==0) {              // if the counter is zero, the temporary movie name is copied on the place of the movie name we selected
            ...
            for (j=0;j<num_genres;j++) {                        // Checks if the genre of the movie already exists in the available genres
                if (strcmp(vec_movies[i].genre,genres[j]) == 0) chkg++;
            }
            if (chkg==0 && vec_movies[i].genre[0]!=0) {         // If it doesn't exist, it gets added
                strcpy(genres[num_genres],vec_movies[i].genre);
                num_genres++;
            }
        }
        else if (ac=='N' || ac=='n') no_changes();
    ...
    if (i>0 && i<=num_actors) {                                // Checks if the option is valid
        ...
        if (ac=='Y' || ac=='y') {
            ...
            for (j=0;j<num_actors;j++) {
                if ((strcmp(tg,vec_actors[j].a_name) == 0) && (strcmp(tg,ct) != 0)) exst++; // if new name is found in other entries and is different than the current name, then increase the counter
            }
            if (exst>0) {                    // if the counter is bigger than zero, then a duplicate entry was found, a message is displayed and nothing is changed
                printf("\n\t\tActor '%s' already exists!",tg);
                no_changes();
            }
            else if (exst==0 && empt==0) {              // if the counter is zero, the temporary actor name is copied on the place of the actor name we selected
...
void list_actors() // List of all the actors
{
    ...
    printf("\t\t"); for (int a=0;a<WIDTH+2;a++) printf("%c",196);
    for (i=0; i<num_actors; i++)
        print_actor(&vec_actors[i]);
    printf("\n\t\t"); for (int a=0;a<WIDTH+2;a++) printf("%c",196);
}
    ...
    if (i>0 && i<=num_genres) {                             // Checks if the input is valid
        ...
        if (ac=='Y' || ac=='y') {
            ...
            for (j=0;j<num_genres;j++) {
                if ((strcmp(tg,genres[j]) == 0) && (strcmp(tg,ct) != 0)) exst++; // if new genre is found in other entries and is different than the current genre, then increase the counter
            }
            if (exst>0) {            // if the counter is bigger than zero, then a duplicate entry was found, a message is displayed and nothing is changed
                printf("\n\t\tGenre '%s' already exists!",tg);
                no_changes();
            }
            else if (exst==0 && empt==0) {      // if the counter is zero, we copy the temporary genre name on the place of the genre we selected
